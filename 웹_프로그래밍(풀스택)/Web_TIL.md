...
구조화 설계는 마치 문서를 쓴다고 생각하면 된다.

현업에서는 Presentation 문서형태의 기획서나 디자인 파일을 받아서 그것을 기반으로 HTML을 개발을 시작한다.

즉 어떠한 화면을 보면서 그대로 구현하는 것이다.

그 화면을 보면서 구조를 분석해야 한다.

먼저 영역을 나눠서 **상단/본문/네비게이션** 이런 식으로 큰 부분부터 분리한다.

그 뒤에 각 영역 안에 내용의 구조를 잡는 것이 일반적이다.

각 영역 안의 내용 역시 여러 가지 형태이다.

목록을 나타내거나, 이미지를 나타내거나, 문단을 나타낼 수 있다.
...
[HTML 구조 링크](https://www.google.com/search?q=html+%EA%B5%AC%EC%A1%B0&tbm=isch&source=iu&ictx=1&fir=dV4hMP_WvYAXOM%252CWZNy6KYdm9kXvM%252C_&vet=1&usg=AI4_-kSJAMgmfAIMQmC147QgeXwwczl89g&sa=X&ved=2ahUKEwiCvIvK9PDtAhVLBogKHfs1B1MQ9QF6BAgPEAE&biw=1073&bih=950#imgrc=dV4hMP_WvYAXOM)
